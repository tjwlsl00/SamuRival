...
{
    // 게임 상태 
    public enum GameDirection { TurnDecide, Playing, LastSpurt, GameEnd };
    ...
    [Header("거리 관련 변수")]
    public float redDecideDist = 0f;
    public float blueDecideDist = 0f;
    public float redLastSpurtDist = 0f;
    public float blueLastSpurtDist = 0f;
    ...
    void Update()
    {
        if (gameDirection == GameDirection.TurnDecide || gameDirection == GameDirection.Playing || gameDirection == GameDirection.LastSpurt)
        {
            // 게임 일시정지
    ...
    public void MeasureDeciderPosition(int gameIndex)
    {
        ...
        if (gameIndex == 0)
        {
            ...
            // 순서 결정 
            TurnDecide(redDecideDist, blueDecideDist);
        }
        // -------
        // 라스트 스퍼트 
        // -------
        else
        {
            ...
            if (redLastSpurtDist > blueLastSpurtDist && redLastSpurtDist != -1f && blueLastSpurtDist != -1f)
            {
            ...
            else if (redLastSpurtDist < blueLastSpurtDist && redLastSpurtDist != -1f && blueLastSpurtDist != -1f)
            {
    ...
    {
        gameDirection = GameDirection.GameEnd;
