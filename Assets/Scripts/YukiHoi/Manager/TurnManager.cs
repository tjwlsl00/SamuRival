...
public class TurnManager : MonoBehaviour
{
    ...
    public IEnumerator ResetTurn(bool SwitchTurn, bool StunTurn)
    {
        ...
        if (!SwitchTurn && !StunTurn)
        {
            ...
            if (red.currentPlayerDirection == Player.PlayerDirection.Attack)
            {
                if (blueState.isHit && !blueState.Item2Effect)
                {
                    isAttackSucess = true;
                    // 공격 성공 효과
            ...
            {
                if (redState.isHit && !redState.Item2Effect)
                {
                    isAttackSucess = true;
                    // 공격 성공 효과
            ...
            else
            {
                ...
                if (redState.isHit) StartCoroutine(redState.ResetHitBool());
                if (blueState.isHit) StartCoroutine(blueState.ResetHitBool());
            }
    ...
    public void CheckFinalWinner()
    {
        if (redState.currentHP > 0 && blueState.currentHP <= 0)
        {
            ...
            // 점수 동기화 
            Global_ScoreManger.Instance.AddScore(0);
        }
        else if (blueState.currentHP > 0 && redState.currentHP <= 0)
        {
            ...
            // 점수 동기화 
            Global_ScoreManger.Instance.AddScore(1);
        }

        // 만약 점수가 둘 중 하나 최고점수에 도달했을때
