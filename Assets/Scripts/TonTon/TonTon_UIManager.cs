...
public class TonTon_UIManager : MonoBehaviour
{
    ...
    [SerializeField] GameObject BonusTimePanel;
    ...
    [SerializeField] float pulseSize = 0.2f;
    ...
    private bool hasBonusTimeUsed = false;
    ...
    void Start()
    {
        ...
        BonusTimePanel.SetActive(false);
    ...
    void Update()
    {
        if (isTimeOver || TonTon_GameManager.Instance.tonton_GameDirection == TonTon_GameManager.TonTon_GameDirection.End) return;
        ...
        Input.GetKeyDown(KeyCode.S) || Input.GetKeyDown(KeyCode.D))
            {
                ApplyPanelShake(BlueScreen);
            }
        }
    }

    #region 타이머 (시작/중지/보너스), 텍스트
    void Timer()
    {
        CurrentTime -= Time.deltaTime;

        if (CurrentTime <= 5f && CurrentTime > EndTime)
        {
            tonTon_SoundManger.PlayLeftTimeClip();
    ...
    public void StopTimer()
    {
        ...
        // 플레이어 게이지 차 
        float diffPlayerGaze = Mathf.Abs(TonTon_GameManager.Instance.CurrentRedGaze - TonTon_GameManager.Instance.CurrentBlueGaze);

        // 보너스 타임을 아직 안 썼고, 점수 차이가 작을 때
        if (!hasBonusTimeUsed && diffPlayerGaze < 10f)
        {
            AddTime();

            // 패널 효과
            VisibleBonusTimePanel();

            // 효과음
            tonTon_SoundManger.PlayBonusTimeClip();
        }
        else
        {
            isTimeOver = true;
            ...
            // UI 업데이트
            UpdateTimerUI();
            TonTon_GameManager.Instance.CheckWinner();
        }
    }

    public void AddTime()
    {
        hasBonusTimeUsed = true;
        // 10초로 타이머 초기화
    ...
    void UpdateTimerUI()
    {
        GameTimer.text = CurrentTime.ToString("F0");

        if (CurrentTime <= 5.5f && CurrentTime > EndTime)
        {
            GameTimer.color = alertColor;

            float scaleUpdate = 1f + Mathf.Sin(Time.time * pulseSpeed) * pulseSize;
            GameTimer.transform.localScale = originalTextScale * scaleUpdate;
        }
        else
        {
            GameTimer.color = Color.black;
            GameTimer.transform.localScale = originalTextScale;
        }
    }
    #endregion

    #region UI(게이지, 커맨드 쉐이크, 망치 질, 보너스/종료 패널)
    // 건설 게이지 
    ...
    public void VisibleBonusTimePanel()
    {
        StartCoroutine(EffectBounsTimePanel(BonusTimePanel));
    }

    IEnumerator EffectBounsTimePanel(GameObject panel)
    {
        yield return null;
    ...
    private void ApplyPanelShake(RectTransform targetPanel)
    {
        if (targetPanel != null)
        {
    ...
    private void UpdateFronzenTimerUI(TextMeshProUGUI targetTimerUI, float remainingTime)
    {
        if (targetTimerUI != null)
            targetTimerUI.text = remainingTime.ToString("F1");

        // 텍스트 
        targetTimerUI.color = Color.black;
