...
public class Map_UIManager : MonoBehaviour
{
    ...
    {
        // 타이틀 오디오 시간 대기 
        if (map_SoundManager.audioSource.isPlaying)
        {
            float titleAudioTime = map_SoundManager.audioSource.clip.length;
            yield return new WaitForSeconds(titleAudioTime);
        }
        ...
        // 소리 연출 대기 
        yield return new WaitForSeconds(map_SoundManager.audioSource.clip.length);
