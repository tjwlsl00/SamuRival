...
public class IceSumo_UIManager : MonoBehaviour
{
    ...
    [SerializeField] GameObject[] finalPanels;
    // 플레이어 
    ...
    void Update()
    {
        if (iceSumo_GameManager.gameDirection == IceSumo_GameManager.GameDirection.Ready || iceSumo_GameManager.gameDirection == IceSumo_GameManager.GameDirection.Play)
        {
            // 메뉴 패널 토글 
        ...
        else if (iceSumo_GameManager.gameDirection == IceSumo_GameManager.GameDirection.Play)
        {
            // 라운드 패널
    ...
    {
        SetKeyIconColor(0, Input.GetKey(KeyCode.RightControl));
        SetKeyIconColor(1, Input.GetKey(KeyCode.LeftControl));
    }
    private void SetKeyIconColor(int keyIndex, bool isCharging)
    {
        if (isCharging)
        {
    ...
    public void FinalUISetting()
    {
        ...
        // 파이널
        GameObject targetFinalPanel = iceSumo_RoundManager.isRedWin ? finalPanels[0] : finalPanels[1];
        RectTransform rect = targetFinalPanel.GetComponent<RectTransform>();

        //크기를 0으로 만들고 활성화
        rect.localScale = Vector3.zero;
        targetFinalPanel.SetActive(true);
