...
public class Stone : MonoBehaviour
{
    ...
    public enum Category { DecideStone, Stone, LastSpurtStone }
    ...
    {
        // 게임 일시정지
        if (Archering_GameManager.Instance.isPaused) return;
        ...
        if (isMyTurn)
        {
            if (!Archering_CameraManager.Instance.isUpCam)
            {
                SwitchStoneState();
            }
        }
                ...
                if (stopTimer >= stopDuration)
                {
                    ...
                    if (category == Category.DecideStone)
                    {
                        ...
                        if (myTeam == Team.RedStone)
                        {
                            ...
                            // 거리 저장 
                            // --------
                            Vector2 center2D = new Vector2(Archering_GameManager.Instance.centerPoint.position.x, Archering_GameManager.Instance.centerPoint.position.z);

                            Vector2 stone2D = new Vector2(gameObject.transform.position.x, gameObject.transform.position.z);

                            float distance = Vector2.Distance(center2D, stone2D);

                            Archering_GameManager.Instance.redDecideDist = distance;

                            // 던짐 확인
                            Archering_GameManager.Instance.isD_RThrowed = true;

                            // 거리 기록
                            Archering_UIManager.Instance.UpdateDistUI(0, 0);
                        ...
                        else
                        {
                            ...
                            Archering_GameManager.Instance.blueDecideDist = distance;

                            // 던짐 확인 
                            Archering_GameManager.Instance.isD_BThrowed = true;

                            // 거리 기록
                            Archering_UIManager.Instance.UpdateDistUI(0, 1);
                    ...
                    // 라스트 스퍼트
                    else if (category == Category.LastSpurtStone)
                    {
                        // 던진 상태 업데이트
                        if (myTeam == Team.RedStone)
                        {
                            ...
                            Archering_GameManager.Instance.redLastSpurtDist = distance;

                            // 던짐 확인
                            Archering_GameManager.Instance.isL_RThrowed = true;

                            // 거리 기록
                            Archering_UIManager.Instance.UpdateDistUI(1, 0);

                            // 디사이드 블루 스톤 스폰 
                            SpawnManager.Instance.SetUpLastSpurtStone(1);
                        ...
                        else
                        {
                            ...
                            Archering_GameManager.Instance.blueLastSpurtDist = distance;

                            // 던짐 확인
                            Archering_GameManager.Instance.isL_BThrowed = true;

                            // 거리 기록
                            Archering_UIManager.Instance.UpdateDistUI(1, 1);
