...
public class TonTon_CommandManager : MonoBehaviour
{
    ...
    {
        // 게임 일시정지
        if(TonTon_GameManager.Instance.isPaused) return;
    ...
    void CheckCommand(TonTon_Player player, CommandType input)
    {
        if (player.tonton_TeamSide == TonTon_TeamSide.Red)
        {
            ...
            else
            {
                ...
                // 사운드 효과 
                tonTon_SoundManger.PlayWrongClip();
            }
        }
        else
        {
    ...
    void OnLineComplete(TonTon_TeamSide side)
    {
        ...
        if (side == TonTon_TeamSide.Red)
        {
            ...
            currentGaze = Mathf.Clamp(currentGaze + addValue, 0f, 100f);
            TonTon_GameManager.Instance.CurrentRedGaze = currentGaze;
        ...
        else
        {
            ...
            currentGaze = Mathf.Clamp(currentGaze + addValue, 0f, 100f);
            TonTon_GameManager.Instance.CurrentBlueGaze = currentGaze;
